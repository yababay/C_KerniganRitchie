...
int strdcasecmp(const char *, const char *);
    ...
    if ((nlines = readlines(lineptr, MAXLINES)) >= 0) {
        qsort_c(lineptr, 0, nlines-1,
                (int (*)(const char*, const char*))(numeric ? numcmp :
                    (fold ? (directory ? strdcasecmp : strcasecmp) :
                     (directory ? strdcmp : strcmp))),
                reverse);
...
int strdcasecmp(const char *s, const char *t) {
    int sf, tf;
...
int strdcmp(const char *s, const char *t) {
    for(;;++s, ++t) {
        while (!isalnum(*s) && !isspace(*s) && *s) {
            ++s;
        }
